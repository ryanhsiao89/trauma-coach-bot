...
def send_message_safely(text):
    ...
    加入 system_instruction 防護機制，並修正 API 歷史紀錄順序錯誤。
    """
    time.sleep(1) 
    ...
    # 建立純對話歷史！
    # 【關鍵修正】：保留第一筆 sys_prompt 讓歷史以 user 開頭，符合 API 規定。
    # 同時排除「最後一筆」([:-1])，因為最後一筆正是我們現在要送出的 text，避免重複發送！
    gemini_history = []
    ...
    for i in range(total_keys):
        ...
        try:
            ...
            model = genai.GenerativeModel(
                ...
                system_instruction=system_prompt, # <--- 鎖定角色的防護！
                safety_settings={
